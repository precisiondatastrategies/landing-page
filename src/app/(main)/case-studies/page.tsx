"use client"

import Link from "next/link"
import { motion } from "framer-motion"
import { ArrowRight } from "lucide-react"
import FinalCTA from "@/components/FinalCTA"

const caseStudies = [
    {
        slug: "greenline-mechanical",
        label: "Illustrative Example",
        number: "01",
        industry: "Commercial HVAC & Plumbing",
        company: "Apex Mechanical Services — PORTFOLIO ILLUSTRATION ONLY",
        sponsor: "Ridgecrest Capital Partners",
        headline: "$1.8M Annual EBITDA Added",
        description:
            "A $48M PE-backed HVAC platform was bleeding revenue through missed calls and manual dispatch. PDS deployed a 24/7 AI voice agent, automated scheduling, and a commercial lead qualification engine — adding $15.3M to the projected exit valuation.",
        metrics: [
            { label: "EBITDA Added", value: "$1.78M", sub: "annualized" },
            { label: "Lead Response", value: "Priority", sub: "from delayed follow-up" },
            { label: "After-Hours Booking", value: "71%", sub: "from 31%" },
            { label: "Exit Value Added", value: "$15.3M", sub: "at 8.5x" },
        ],
        tags: ["AI Voice Agent", "Lead Qualification", "Scheduling Automation", "PE Dashboard"],
        engagement: "$110K total engagement",
        roi: "139x ROI",
    },
]

export default function CaseStudiesPage() {
    return (
        <main className="min-h-screen bg-slate-50 font-sans">

            {/* Hero */}
            <section className="pt-36 pb-20 border-b border-slate-200 bg-white">
                <div className="max-w-6xl mx-auto px-5 lg:px-6 xl:px-8">
                    <motion.div
                        initial={{ opacity: 0, y: 16 }}
                        animate={{ opacity: 1, y: 0 }}
                        transition={{ duration: 0.5 }}
                    >
                        <p className="text-xs font-semibold uppercase tracking-widest text-blue-600 mb-5">Case Studies</p>
                        <h1 className="text-5xl lg:text-6xl font-bold text-slate-900 leading-[1.02] tracking-tight mb-6 max-w-3xl">
                            AI that moves<br />the exit number.
                        </h1>
                        <p className="text-xl text-slate-500 max-w-2xl leading-relaxed">
                            Every engagement is measured in EBITDA impact, not deliverables. Here&apos;s what that looks like in practice.
                        </p>
                    </motion.div>
                </div>
            </section>

            {/* Case Study Cards */}
            <section className="py-20">
                <div className="max-w-6xl mx-auto px-5 lg:px-6 xl:px-8 space-y-6">
                    {caseStudies.map((cs, i) => (
                        <motion.div
                            key={cs.slug}
                            initial={{ opacity: 0, y: 24 }}
                            whileInView={{ opacity: 1, y: 0 }}
                            viewport={{ once: true }}
                            transition={{ duration: 0.5, delay: i * 0.08 }}
                        >
                            <Link href={`/case-studies/${cs.slug}`} className="group block">
                                <div className="relative overflow-hidden border border-slate-200 rounded-3xl p-8 lg:p-12 bg-white shadow-sm hover:shadow-xl hover:shadow-slate-200/70 hover:-translate-y-0.5 transition-all duration-300">
                                    <div className="absolute -top-24 -right-20 w-72 h-72 bg-linear-to-br from-blue-100/50 via-cyan-100/30 to-transparent rounded-full pointer-events-none" />

                                    {/* Top meta row */}
                                    <div className="relative z-10 flex flex-wrap items-center gap-3 mb-8">
                                        <span className="text-5xl font-black text-slate-100 select-none mr-2">{cs.number}</span>
                                        <span className="px-3 py-1 rounded-full bg-amber-50 border border-amber-200 text-amber-700 text-xs font-semibold">
                                            {cs.label}
                                        </span>
                                        <span className="text-slate-400 text-sm">{cs.industry}</span>
                                        <span className="text-slate-200">·</span>
                                        <span className="text-slate-400 text-sm">{cs.sponsor}</span>
                                    </div>

                                    <div className="relative z-10 flex flex-col lg:flex-row lg:items-start gap-10 lg:gap-14">
                                        {/* Left */}
                                        <div className="lg:flex-1">
                                            <p className="text-xs font-bold uppercase tracking-[0.16em] text-blue-600/90 mb-3">{cs.company}</p>
                                            <h2 className="text-4xl lg:text-5xl font-bold text-slate-900 leading-[1.04] mb-4 group-hover:text-blue-700 transition-colors duration-200">
                                                {cs.headline}
                                            </h2>
                                            <p className="text-slate-500 leading-relaxed mb-7 text-[1.02rem] max-w-xl">
                                                {cs.description}
                                            </p>

                                            <div className="flex flex-wrap gap-2 mb-7">
                                                {cs.tags.map(tag => (
                                                    <span key={tag} className="px-3 py-1 rounded-full bg-slate-50 border border-slate-200 text-slate-600 text-xs font-medium">
                                                        {tag}
                                                    </span>
                                                ))}
                                            </div>

                                            <div className="flex items-center gap-5 text-sm">
                                                <span className="text-slate-400">{cs.engagement}</span>
                                                <span className="font-semibold text-emerald-600">{cs.roi}</span>
                                            </div>
                                        </div>

                                        {/* Right — metrics */}
                                        <div className="grid grid-cols-2 gap-4 lg:w-100 shrink-0">
                                            {cs.metrics.map(m => (
                                                <div key={m.label} className="bg-white border border-slate-200 rounded-2xl p-6 min-h-[190px] shadow-sm group-hover:border-slate-300 transition-colors overflow-hidden flex flex-col">
                                                    <div className={`${m.value.length > 8 ? "text-[1.65rem] lg:text-[1.8rem]" : "text-[2rem] lg:text-[2.2rem]"} leading-[1.08] font-bold tracking-tight text-slate-900 mb-3`}>{m.value}</div>
                                                    <div className="text-[0.95rem] text-emerald-600 font-semibold leading-tight mb-3">{m.sub}</div>
                                                    <div className="text-[0.9rem] leading-snug text-slate-400 mt-auto">{m.label}</div>
                                                </div>
                                            ))}
                                        </div>
                                    </div>

                                    {/* Bottom CTA row */}
                                    <div className="relative z-10 mt-9 pt-6 border-t border-slate-100 flex items-center justify-between">
                                        <span className="text-slate-400 text-sm">Engagement example · HVAC / Plumbing</span>
                                        <span className="inline-flex items-center gap-1.5 text-blue-700 font-semibold text-sm group-hover:gap-2.5 transition-all duration-200">
                                            Read full case study <ArrowRight className="w-4 h-4" />
                                        </span>
                                    </div>
                                </div>
                            </Link>
                        </motion.div>
                    ))}

                    {/* Coming soon */}
                    <motion.div
                        initial={{ opacity: 0, y: 24 }}
                        whileInView={{ opacity: 1, y: 0 }}
                        viewport={{ once: true }}
                        transition={{ duration: 0.5, delay: 0.15 }}
                        className="border border-dashed border-slate-300 rounded-3xl p-10 text-center bg-white"
                    >
                        <p className="text-xs font-semibold uppercase tracking-widest text-slate-300 mb-3">Coming Soon</p>
                        <p className="text-slate-400 text-base">More case studies in progress — pest control, dental DSO, commercial cleaning.</p>
                    </motion.div>
                </div>
            </section>

            <FinalCTA />
        </main>
    )
}
